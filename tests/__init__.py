...
""" Unit tests for r128gain module. """
...
from r128gain import gain_to_scale
...
class TestR128Gain(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        download("http://www.largesound.com/ashborytour/sound/brobob.mp3", mp3_filepath)

        invalid_mp3_filepath = os.path.join(cls.ref_temp_dir.name, "invalid.mp3")
        download("https://www.dropbox.com/s/bdm7wyqaj3ij8ci/Death%20Star.mp3?dl=1", invalid_mp3_filepath)
        ...
        aac_filepath = os.path.join(cls.ref_temp_dir.name, "f.aac")
        download("https://dl.espressif.com/dl/audio/ff-16b-2c-44100hz.aac", aac_filepath)

        flac_filepath = os.path.join(cls.ref_temp_dir.name, "f.flac")
        flac_zic_filepath = f"{flac_filepath}.zip"
        download(
            "http://helpguide.sony.net/high-res/sample1/v1/data/Sample_HisokanaMizugame_88_2kHz24bit.flac.zip",
            flac_zic_filepath,
        )
        ...
        flac_zic_filepath = f"{flac_filepath}.zip"
        download("https://github.com/desbma/r128gain/files/3006101/snippet_with_high_true_peak.zip", flac_zic_filepath)
        with zipfile.ZipFile(flac_zic_filepath) as zip_file:
            in_filename = zip_file.namelist()[0]
            with zip_file.open(in_filename) as in_file:
                with open(flac_filepath, "wb") as out_file:
        ...
        download(
            "https://www.dropbox.com/s/hnkmioxwu56dgs0/04-There%27s%20No%20Other%20Way.mp3?dl=1", silence_mp3_filepath
        )

        high_peak_flac_filepath = os.path.join(cls.ref_temp_dir.name, "high_peak.flac")
        download("https://www.dropbox.com/s/ev5rqjs00pj6kpd/loudness%20war.flac?dl=1", high_peak_flac_filepath)
    ...
    def setUp(self):
        ...
        self.high_peak_flac_filepath = os.path.join(self.temp_dir2.name, "high_peak.flac")

        self.ref_levels = {
            self.vorbis_filepath: (-7.7, 0),
            self.opus_filepath: (-14.7, 0),
            self.mp3_filepath: (-13.9, -0.5),
            self.m4a_filepath: (-20.6, 0),
            self.aac_filepath: (-9.0, 0),
            self.flac_filepath: (-26.8, -12.7),
            self.wv_filepath: (-3.2, -3),
            0: (-10.6, 1.0),
        }
        self.ref_levels_2 = self.ref_levels.copy()
        self.ref_levels_2.update(
            {
                self.flac_filepath_2: (-6.2, 0),
                self.silence_wv_filepath: (-70.0, -90.3),
                self.silence_mp3_filepath: (-70.0, 0),
                0: (-10.3, 0),
            }
        )
        self.ref_levels_alt = {
            self.flac_filepath_weird_sample_rate: (-14.1, -1.4),
            self.high_peak_flac_filepath: (-2.1, 0),
        }
    ...
    def test_scan(self):
        ...
        for album_gain in (False, True):
            ...
            vals = r128gain.scan(filepaths, album_gain=album_gain)
            for key in ref_levels:
                with self.subTest(key=key):
                    self.assertAlmostEqual(vals[key][0], ref_levels[key][0], places=6)
                    self.assertAlmostEqual(vals[key][1], ref_levels[key][1], places=6)

            # opus only files
            filepaths = (self.opus_filepath, self.opus_filepath2)
            ref_levels_opus = {
                self.opus_filepath: self.ref_levels[self.opus_filepath],
            ...
            if album_gain:
                ...
                filepaths = (
                    self.opus_filepath,  # reduce permutation counts to speed up tests
                    self.m4a_filepath,
                    self.mp3_filepath,
                )
                ref_levels = r128gain.scan(filepaths, album_gain=True)
                if IS_TRAVIS:
    ...
    def test_tag(self):
        ...
        for i, delete_tags in zip(range(3), (False, True, False)):
            ...
            with self.subTest(iteration=i + 1, delete_tags=delete_tags):
                ...
                self.assertEqual(mf["REPLAYGAIN_TRACK_GAIN"], [f"{expected_track_gain_rg2:.2f} dB"])
                self.assertIn("REPLAYGAIN_TRACK_PEAK", mf)
                self.assertEqual(mf["REPLAYGAIN_TRACK_PEAK"], [f"{peak:.8f}"])
                ...
                self.assertEqual(mf["TXXX:REPLAYGAIN_TRACK_GAIN"].text, [f"{expected_track_gain_rg2:.2f} dB"])
                self.assertIn("TXXX:REPLAYGAIN_TRACK_PEAK", mf)
                self.assertEqual(mf["TXXX:REPLAYGAIN_TRACK_PEAK"].text, [f"{peak:.6f}"])
                ...
                if delete_tags:
                    ...
                    self.assertNotIn("REPLAYGAIN_TRACK_GAIN", mf)
                    self.assertNotIn("REPLAYGAIN_TRACK_PEAK", mf)
                r128gain.tag(self.aac_filepath, loudness, peak)
                mf = mutagen.File(self.aac_filepath)
                self.assertIsInstance(mf.tags, mutagen.apev2.APEv2)
                self.assertIn("REPLAYGAIN_TRACK_GAIN", mf)
                self.assertEqual(str(mf["REPLAYGAIN_TRACK_GAIN"]), f"{expected_track_gain_rg2:.2f} dB")
                self.assertIn("REPLAYGAIN_TRACK_PEAK", mf)
                self.assertEqual(str(mf["REPLAYGAIN_TRACK_PEAK"]), f"{peak:.8f}")
            ...
            for album_gain in (False, True):
                with self.subTest(i=i, skip_tagged=skip_tagged, album_gain=album_gain), unittest.mock.patch(
                    ...
                    if skip_tagged and (i > 0):
                        ...
                        self.assertEqual(get_r128_loudness_mock.call_count, 8)
                    ...
                    self.assertIn("REPLAYGAIN_TRACK_GAIN", mf)
                    self.assertEqual(
                        mf["REPLAYGAIN_TRACK_GAIN"],
                        [f"{ref_loudness_rg2 - self.ref_levels[self.vorbis_filepath][0]:.2f} dB"],
                    )
                    self.assertIn("REPLAYGAIN_TRACK_PEAK", mf)
                    self.assertEqual(
                        mf["REPLAYGAIN_TRACK_PEAK"], [f"{gain_to_scale(self.ref_levels[self.vorbis_filepath][1]):.8f}"]
                    )
                    if album_gain:
                        self.assertIn("REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertEqual(
                            mf["REPLAYGAIN_ALBUM_GAIN"],
                        ...
                        self.assertEqual(
                            ...
                            [f"{gain_to_scale(self.ref_levels[self.max_peak_filepath][1]):.8f}"],
                        )
                    elif i == 0:
                    ...
                    if album_gain:
                        ...
                        self.assertEqual(
                            ...
                            [
                                str(
                                    r128gain.float_to_q7dot8(
                                        ref_loudness_opus - self.ref_levels[r128gain.ALBUM_GAIN_KEY][0]
                                    )
                                )
                            ],
                        )
                    ...
                    self.assertIn("TXXX:REPLAYGAIN_TRACK_GAIN", mf)
                    self.assertEqual(
                        mf["TXXX:REPLAYGAIN_TRACK_GAIN"].text,
                        [f"{ref_loudness_rg2 - self.ref_levels[self.mp3_filepath][0]:.2f} dB"],
                    )
                    self.assertIn("TXXX:REPLAYGAIN_TRACK_PEAK", mf)
                    self.assertEqual(
                        mf["TXXX:REPLAYGAIN_TRACK_PEAK"].text,
                        [f"{gain_to_scale(self.ref_levels[self.mp3_filepath][1]):.6f}"],
                    )
                    if album_gain:
                        self.assertIn("TXXX:REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertEqual(
                            mf["TXXX:REPLAYGAIN_ALBUM_GAIN"].text,
                            [f"{ref_loudness_rg2 - self.ref_levels[r128gain.ALBUM_GAIN_KEY][0]:.2f} dB"],
                        )
                        self.assertIn("TXXX:REPLAYGAIN_ALBUM_PEAK", mf)
                        self.assertEqual(
                            mf["TXXX:REPLAYGAIN_ALBUM_PEAK"].text,
                            [f"{gain_to_scale(self.ref_levels[self.max_peak_filepath][1]):.6f}"],
                        )
                    elif i == 0:
                    ...
                    self.assertEqual(
                        ...
                        f"{gain_to_scale(self.ref_levels[self.m4a_filepath][1]):.6f}",
                    )
                    if album_gain:
                    ...
                    self.assertIn("REPLAYGAIN_TRACK_GAIN", mf)
                    self.assertEqual(
                        str(mf["REPLAYGAIN_TRACK_GAIN"]),
                        f"{ref_loudness_rg2 - self.ref_levels[self.aac_filepath][0]:.2f} dB",
                    )
                    self.assertIn("REPLAYGAIN_TRACK_PEAK", mf)
                    self.assertEqual(
                        str(mf["REPLAYGAIN_TRACK_PEAK"]), f"{gain_to_scale(self.ref_levels[self.aac_filepath][1]):.8f}"
                    )
                    if album_gain:
                        self.assertIn("REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertEqual(
                            str(mf["REPLAYGAIN_ALBUM_GAIN"]),
                            f"{ref_loudness_rg2 - self.ref_levels[r128gain.ALBUM_GAIN_KEY][0]:.2f} dB",
                        )
                        self.assertIn("REPLAYGAIN_ALBUM_PEAK", mf)
                        self.assertEqual(
                            str(mf["REPLAYGAIN_ALBUM_PEAK"]),
                            f"{gain_to_scale(self.ref_levels[self.max_peak_filepath][1]):.8f}",
                        )
                    elif i == 0:
                        self.assertNotIn("REPLAYGAIN_ALBUM_GAIN", mf)
                    ...
                    self.assertEqual(
                        mf["REPLAYGAIN_TRACK_GAIN"],
                        [f"{ref_loudness_rg2 - self.ref_levels[self.flac_filepath][0]:.2f} dB"],
                    )
                    self.assertIn("REPLAYGAIN_TRACK_PEAK", mf)
                    self.assertEqual(
                        mf["REPLAYGAIN_TRACK_PEAK"], [f"{gain_to_scale(self.ref_levels[self.flac_filepath][1]):.8f}"]
                    )
                    if album_gain:
                        self.assertIn("REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertEqual(
                    ...
                    self.assertEqual(
                        str(mf["REPLAYGAIN_TRACK_GAIN"]),
                        f"{ref_loudness_rg2 - self.ref_levels[self.wv_filepath][0]:.2f} dB",
                    )
                    self.assertIn("REPLAYGAIN_TRACK_PEAK", mf)
                    self.assertEqual(
                        str(mf["REPLAYGAIN_TRACK_PEAK"]), f"{gain_to_scale(self.ref_levels[self.wv_filepath][1]):.8f}"
                    )
                    if album_gain:
                        self.assertIn("REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertEqual(
    ...
    def test_process_recursive(self):  # noqa: C901
        ...
        os.mkdir(album_dir1)
        album1_vorbis_filepath = os.path.join(album_dir1, os.path.basename(self.vorbis_filepath))
        shutil.copyfile(self.vorbis_filepath, album1_vorbis_filepath)
        album1_opus_filepath = os.path.join(album_dir1, os.path.basename(self.opus_filepath))
        shutil.copyfile(self.opus_filepath, album1_opus_filepath)

        album_dir2 = os.path.join(self.temp_dir.name, "b")
        os.mkdir(album_dir2)
        album2_mp3_filepath = os.path.join(album_dir2, os.path.basename(self.mp3_filepath))
        shutil.copyfile(self.mp3_filepath, album2_mp3_filepath)
        album2_m4a_filepath = os.path.join(album_dir2, os.path.basename(self.m4a_filepath))
            ...
            for album_gain in (False, True):
                with self.subTest(i=i, skip_tagged=skip_tagged, album_gain=album_gain), unittest.mock.patch(
                    ...
                    if skip_tagged and (i > 0):
                        ...
                        self.assertEqual(get_r128_loudness_mock.call_count, 17)
                    elif album_gain and not skip_tagged:
                        self.assertEqual(get_r128_loudness_mock.call_count, 17)
                    else:
                        self.assertEqual(get_r128_loudness_mock.call_count, 14)
                    ...
                    if album_gain:
                        ...
                        ref_q7dot8 = r128gain.float_to_q7dot8(
                            ref_loudness_opus - self.ref_levels[r128gain.ALBUM_GAIN_KEY][0]
                        )
                    ...
                    if album_gain:
                        ...
                        self.assertGainStrAlmostEqual(
                            mf["TXXX:REPLAYGAIN_ALBUM_GAIN"].text[0],
                            ref_loudness_rg2 - self.ref_levels[r128gain.ALBUM_GAIN_KEY][0],
                        )
                        self.assertIn("TXXX:REPLAYGAIN_ALBUM_PEAK", mf)
                        self.assertEqual(
                    ...
                    if album_gain:
                        self.assertIn("REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertEqual(
                            mf["REPLAYGAIN_ALBUM_GAIN"], [f"{ref_loudness_rg2 - ref_levels_dir1[0]:.2f} dB"]
                        )
                        self.assertIn("REPLAYGAIN_ALBUM_PEAK", mf)
                        self.assertEqual(mf["REPLAYGAIN_ALBUM_PEAK"], [f"{gain_to_scale(ref_levels_dir1[1]):.8f}"])
                    elif i == 0:
                        self.assertNotIn("REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertNotIn("REPLAYGAIN_ALBUM_PEAK", mf)
                    ...
                    if album_gain:
                        self.assertIn("TXXX:REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertEqual(
                            mf["TXXX:REPLAYGAIN_ALBUM_GAIN"].text, [f"{ref_loudness_rg2 - ref_levels_dir2[0]:.2f} dB"]
                        )
                        self.assertIn("TXXX:REPLAYGAIN_ALBUM_PEAK", mf)
                        self.assertEqual(
                            mf["TXXX:REPLAYGAIN_ALBUM_PEAK"].text, [f"{gain_to_scale(ref_levels_dir2[1]):.6f}"]
                        )
                    elif i == 0:
                        self.assertNotIn("TXXX:REPLAYGAIN_ALBUM_GAIN", mf)
                        self.assertNotIn("TXXX:REPLAYGAIN_ALBUM_GAIN", mf)
                    ...
                    if album_gain:
                        ...
                        self.assertEqual(
                            bytes(mf["----:com.apple.iTunes:replaygain_album_peak"][0]).decode(),
                            f"{gain_to_scale(ref_levels_dir2[1]):.6f}",
                        )
                    elif i == 0:
                        self.assertNotIn("----:com.apple.iTunes:replaygain_album_gain", mf)
                        self.assertNotIn("----:com.apple.iTunes:replaygain_album_peak", mf)
    ...
    def test_tag_oggopus_output_gain(self):
        ...
        opus_filepath = shutil.copyfile(
            self.opus_filepath, ".".join((os.path.splitext(self.opus_filepath)[0], "2" ".opus"))
        )
        ...
        self.assertEqual(r128gain.scan([opus_filepath]), {opus_filepath: (float(ref_loudness_opus), ref_track_peak)})

        opus_filepath = shutil.copyfile(
            self.opus_filepath, ".".join((os.path.splitext(self.opus_filepath)[0], "2" ".opus"))
        )
        r128gain.tag(opus_filepath, None, None, album_loudness=ref_album_loudness, opus_output_gain=True)
        mf = mutagen.File(opus_filepath)
        self.assertIsInstance(mf.tags, mutagen._vorbis.VComment)
        self.assertNotIn("R128_TRACK_GAIN", mf)
        self.assertIn("R128_ALBUM_GAIN", mf)
        ...
        self.assertEqual(
            r128gain.scan([opus_filepath]),
            {
                opus_filepath: (
                    float(ref_loudness_opus - track_to_album_gain_delta),
                    ref_track_peak - track_to_album_gain_delta,
                )
            },
        )
        ...
        r128gain.tag(opus_filepath, ref_track_loudness, None, album_loudness=ref_album_loudness, opus_output_gain=True)
        mf = mutagen.File(opus_filepath)
        self.assertIsInstance(mf.tags, mutagen._vorbis.VComment)
        self.assertIn("R128_TRACK_GAIN", mf)
